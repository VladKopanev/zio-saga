 ...
 *
 * @see
 *   [[https://blog.couchbase.com/saga-pattern-implement-business-transactions-using-microservices-part/ Saga pattern]]
 *
 * Saga class has three type parameters - R for environment, E for errors and A for successful result. Saga wraps a ZIO
 * that carries the compensating action in both error and result channels and enables a composition with another Sagas
 * in for-comprehensions. If error occurs Saga will execute compensating actions starting from action that corresponds
...
final class Saga[-R, +E, +A] private (
  ...
  def flatMap[R1 <: R, E1 >: E, B](f: A => Saga[R1, E1, B]): Saga[R1, E1, B] =
    new Saga(request.flatMap { case (a, compA) =>
      f(a).request.bimap(
  ...
  /**
   * Returns Saga that will execute this Saga in parallel with other, combining the result in a tuple. Both compensating
   * actions would be executed in case of failure.
   */
...
object Saga {
  ...
  /**
   * Constructs new Saga from action and compensation function that will be applied only to failed result of this
   * request. If given action succeeds associated compensating action would not be executed during the compensation
   * phase.
   */
  ...
  /**
   * Constructs new Saga from action and compensation function that will be applied only to successful result of this
   * request. If given action fails associated compensating action would not be executed during the compensation phase.
   */
  ...
  /**
   * Constructs a Saga that applies the function `f` to each element of the `Iterable[A]` in parallel, and returns the
