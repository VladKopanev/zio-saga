...
sealed abstract class Saga[F[_], A] {
  ...
  def transact(implicit F: MonadError[F, Throwable], C: Concurrent[F]): F[A] = {
    def interpret[X](saga: Saga[F, X]): F[(X, F[Unit])] = saga match {
      ...
      case Par(left: Saga[F, Any], right: Saga[F, Any], combine: ((Any, Any) => X)) =>
        C.racePair(interpret(left), interpret(right)).flatMap {
...
object Saga {
  ...
  def compensate[F[_], A](comp: F[A], compensation: F[Unit]): Saga[F, A] =
    compensateResult(comp, _ => compensation)

  def compensateIfFail[F[_], A](comp: F[A], compensation: Throwable => F[Unit])(F: InvariantMonoidal[F]): Saga[F, A] =
    compensateResult(comp, result => result.fold(compensation, _ => F.unit))

  def compensateIfSuccess[F[_], A](comp: F[A], compensation: A => F[Unit])(F: InvariantMonoidal[F]): Saga[F, A] =
    compensateResult(comp, result => result.fold(_ => F.unit, compensation))

  def compensateResult[F[_], A](comp: F[A], compensation: Either[Throwable, A] => F[Unit]): Saga[F, A] =
  ...
  /**
    ...
    * the results.
    */
  def collectAllPar[F[_] : Applicative, A](sagas: Saga[F, A]*): Saga[F, List[A]] =
    collectAllPar(sagas)
  ...
  implicit class Compensable[F[_], A](val request: F[A]) {
    ...
    def compensateIfFail(compensation: Throwable => F[Unit])(F: InvariantMonoidal[F]): Saga[F, A] =
      Saga.compensateResult(request, result => result.fold(compensation, _ => F.unit))

    def compensateIfSuccess(compensation: A => F[Unit])(F: InvariantMonoidal[F]): Saga[F, A] =
      Saga.compensateResult(request, result => result.fold(_ => F.unit, compensation))

    def compensateResult(compensation: Either[Throwable, A] => F[Unit]): Saga[F, A] =
      Saga.compensateResult(request, compensation)
